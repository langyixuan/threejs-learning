...
void main(){
  ...
  vec3 noise1=(position*vec3(1.,3.,1.)+time*vec3(0.,.4,0.))*vec3(5.,1.,1.);
  vec3 noise2=(position+time*vec3(0.,.6,0.))*vec3(5.,1.,1.);
  
  newpos.x+=.4*progress*cnoise(vec4(noise1,time/5.));
  newpos.yz+=.7*progress*(cnoise(vec4(noise2,time/5.))+.2);
