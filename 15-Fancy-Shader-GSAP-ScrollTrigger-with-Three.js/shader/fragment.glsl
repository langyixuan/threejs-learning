...
void main(){
	...
	vec4 txt=texture2D(fTexture,vUv+.2*cnoise(vec4(vUv*10.,time/10.,0.),vec4(5.)));
	
	// gl_FragColor=vec4(vNormal,1.);
	// gl_FragColor=vec4(abs(sin((diff*2.))));
	// gl_FragColor=txt;
	// gl_FragColor=vec4(fresnel);
	// gl_FragColor=vec4(diff);
	gl_FragColor=vec4(mix(vec3(1.,1.,1.),txt.rgb,fresnel),1.);
}